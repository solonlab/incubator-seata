...
import static org.apache.seata.solon.autoconfigure.StarterConstants.JSON_PREFIX;

/**
 * Seata JSON configuration properties.
 *
 * <p>The {@code serializerType} is consumed by {@code JsonUtil} through the configuration
 * key {@code seata.json.serializer-type} (resolved via {@code SolonConfigurationProvider}),
 * while {@code allowlist} is loaded into {@link JsonAllowlistManager} on init to guard
 * JSON deserialization.</p>
 */
@Configuration
@Inject(value = "${" + JSON_PREFIX + "}", required = false)
public class SeataJsonProperties {
     ...
     * This property is bound from the configuration key {@code seata.json.serializer-type}
